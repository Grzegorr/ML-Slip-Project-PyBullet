...
x = np.zeros((NoOfTrajectories,35,5))
...
for trajectoryNo in range(NoOfTrajectories):
    ...
    WaypointsNoTime1D = np.zeros((35,5))
    for i in range(35):
        temp_waypoint = [WaypointsNoTime[i][0][2],WaypointsNoTime[i][2],WaypointsNoTime[i][3],WaypointsNoTime[i][4],WaypointsNoTime[i][5]]
        #print(temp_waypoint)
...
model = Sequential()
model.add(LSTM(256,return_sequences=True,input_shape = (35,5)))
...
plt.savefig("HistoryPlots/SimplifiedMovementAll.png")

model.save("TrainedNetworks/Simplified1All.h5")
